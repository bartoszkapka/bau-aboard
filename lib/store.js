...
export async function saveQuestion(q) {
  ...
  const record = {
    ...
    type: q.type === "open" ? "open" : "closed",
    text: q.text || "",
    media: q.media && q.media.url ? { type: q.media.type, url: q.media.url } : null,
    options: q.type === "closed" ? (q.options || []).slice(0, 4) : [],
    correctOptionId: q.type === "closed" ? q.correctOptionId || null : null,
    correctAnswer: q.type === "open" ? q.correctAnswer || "" : "",
  };
...
export async function assembleView(code, role, pid) {
  ...
  if (state.final) {
    ...
    finalView = {
      ...
      revealed: !!f.revealed,
      phase: f.phase, // present | collecting | revealing | done
    };
    if (isHost || f.revealed) {
      finalView.correctValue = f.correctValue;
      finalView.manipulated = f.manipulated;
      finalView.manipValue = f.manipValue;
